...
import { logout, setCredentials, updateUser } from '@/modules/auth/slices/authSlice';
 ...
 */
async function refreshPermissionsQuietly() {
...
axiosInstance.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (status >= 500) {
      ...
      const errorData = error.response?.data;
      if (errorData?.message && !originalRequest.url?.includes('login')) {
