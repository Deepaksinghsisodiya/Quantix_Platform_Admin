...
interface ClienteleListProps {
  ...
  onRetry: () => void;
  onOpenAdd: () => void;
  onOpenEdit: (brand: ClientBrand) => void;
  onOpenDelete: (brand: ClientBrand) => void;
...
export const ClienteleList: React.FC<ClienteleListProps> = ({
  brands = [],
  activeTab,
  onTabChange,
  counts,
  ...
  const [statusFilter, setStatusFilter] = useState<'all' | 'featured' | 'live' | 'hidden'>('all');
  ...
  const filteredBrands = useMemo(() => {
    return safeBrands.filter((b) => {
      ...
      const matchesStatus =
        statusFilter === 'all' ||
        (statusFilter === 'featured' && b?.isFeatured) ||
        (statusFilter === 'live' && b?.isActive) ||
        (statusFilter === 'hidden' && !b?.isActive);

      return matchesSearch && matchesStatus;
    });
  }, [safeBrands, searchQuery, statusFilter]);

  const liveCount = useMemo(() => safeBrands.filter((b) => b?.isActive).length, [safeBrands]);
  const featuredCount = useMemo(() => safeBrands.filter((b) => b?.isFeatured).length, [safeBrands]);
  const hiddenCount = useMemo(() => safeBrands.filter((b) => !b?.isActive).length, [safeBrands]);
  ...
  return (
    <div className="w-full space-y-4 sm:space-y-6 animate-fade-in max-w-[1600px] mx-auto px-1 sm:px-2">
      ...
      <ATMPageHeader
        title="Clientele & Brand Partners CMS"
        subtitle="Manage client logos, enterprise partner brands, industry categories, and marquee ribbons across all websites."
        icon={Handshake}
        iconColor="theme"
        action={{
          label: `Add ${activeTab} Brand`,
          onClick: onOpenAdd,
          icon: Plus,
        }}
      />
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-4">
          <div className="rounded-xl border border-slate-200/90 bg-white p-3.5 sm:p-4 shadow-xs dark:border-slate-800 dark:bg-slate-900/60">
            ...
            <div className="mt-1 flex items-baseline justify-between">
              <span className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">{safeBrands.length}</span>
          ...
          <div className="rounded-xl border border-slate-200/90 bg-white p-3.5 sm:p-4 shadow-xs dark:border-slate-800 dark:bg-slate-900/60">
            <div className="text-[11px] sm:text-xs font-medium text-slate-500 dark:text-slate-400">Live in Marquee</div>
            <div className="mt-1 flex items-baseline justify-between">
          ...
          <div className="rounded-xl border border-slate-200/90 bg-white p-3.5 sm:p-4 shadow-xs dark:border-slate-800 dark:bg-slate-900/60">
            <div className="text-[11px] sm:text-xs font-medium text-slate-500 dark:text-slate-400">Featured Brands</div>
            <div className="mt-1 flex items-baseline justify-between">
              <span className="text-xl sm:text-2xl font-bold text-amber-600 dark:text-amber-400">{featuredCount}</span>
      ...
      {isLoading ? (
        <ATMSkeleton
          variant={viewMode === 'grid' ? 'clientele-card' : 'clientele-row'}
          count={6}
        />
      ) : isError ? (
        <ATMCard className="p-8 text-center border-rose-200 dark:border-rose-900/50 bg-rose-50/30 dark:bg-rose-950/20">
          ...
          <h3 className="text-base font-semibold text-slate-900 dark:text-white">Failed to load brand partners</h3>
          <p className="text-xs text-slate-500 mt-1 max-w-md mx-auto">
            Unable to connect to the clientele API service. Please verify your connection or retry.
          </p>
          <div className="mt-4">
        ...
        <ATMCard className="p-8 sm:p-10 text-center border-dashed border-slate-300 dark:border-slate-800">
          ...
          <h3 className="text-base font-semibold text-slate-900 dark:text-white">
            {searchQuery ? 'No matching brands found' : `No client brands yet for ${activeTab}`}
          </h3>
          <p className="text-xs text-slate-500 mt-1 max-w-md mx-auto">
            {searchQuery
              ? 'Try modifying your search keywords or clear the filter.'
              : `Add your first client or partner brand for the ${activeTab} website marquee.`}
          </p>
          {!searchQuery && (
            <div className="mt-5">
        ...
        >
          {filteredBrands.map((brand, idx) => (
            <ClienteleCard
              key={brand?.brandId || brand?.id || idx}
              brand={brand}
              index={idx}
